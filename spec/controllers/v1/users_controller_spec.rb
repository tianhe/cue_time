...
describe V1::UsersController do
  ...
  describe "POST 'create'" do
    it "returns http success if parameters are valid" do
      expect {
        post :create, email: 'joe@schmoe.com', password: 'password', password_confirmation: 'password', name: 'Joe Schmoe', gender: 'M', birthdate: '2014-01-01', drinking_habit: 'heavily'
        response.should be_success
      }.to change{ User.count }.by(1)

      User.first.email.should == 'joe@schmoe.com'
      User.first.name.should == 'Joe Schmoe'
      User.first.gender.should == 'M'
      User.first.birthdate.should == Date.parse('2014-01-01')
      User.first.drinking_habit.should == 'heavily'
      User.first.authentication_token.should_not == nil      

      json = JSON.parse(response.body)
      json.should == { authentication_token: User.first.authentication_token }.with_indifferent_access
    end

    it "returns 400 if the parameters are missing" do
      expect {
        post :create, email: 'joe@schmoe.com'
  ...
  describe "PATCH 'update'" do
    it "returns http success" do
      put :update, id: user.id, gender: 'F', email: user.email, authentication_token: user.authentication_token
      response.should be_success

      User.first.gender.should == 'F'
    end

    it "returns 400 if the id doesnt exist" do
      put :update, id: 0, name: 'Jill', email: user.email, authentication_token: user.authentication_token
      response.code.should == '400'
    end    
  end
end
